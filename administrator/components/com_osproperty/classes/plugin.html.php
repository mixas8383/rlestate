...
class HTML_OspropertyPlugin{
	...
	function listPlugins($option,$rows,$pageNav,$lists){
		...
		JToolBarHelper::title(JText::_('OS_MANAGE_PAYMENT_PLUGINS'),"stack");
		...
		<div id="editcell">
			...
			{
				$row = &$rows[$i];
				$link 	= JRoute::_( 'index.php?option=com_osproperty&task=plugin_edit&cid[]='. $row->id );
					...
					<td>
						...
						?>
							<a href="<?php echo $link; ?>" title="<?php echo $row->title; ?>">
								<img src="<?php echo JUri::root().'images/osproperty/plugins/'.$row->name.'.png'; ?>" width="100" />
							</a>
						<?php } ?>
					</td>
					<td>
	...
	function editPlugin($option,$item,$lists,$form){
		...
		JToolBarHelper::title(JText::_('OS_PLUGIN')." ".$type);
