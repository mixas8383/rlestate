...
class HelperOspropertyFields
{
    ...
    function moduleSearchMselectField($field, $inputbox_width_site)
    {
        ...
        public static function getFieldTypes()
        {
            ...
            {
                $db = JFactory::getDbo();
                $query = "Select * from #__osrs_extra_fields as b"
                        . " WHERE b.published = '1'  order by b.ordering";
                $db->setQuery($query);
                $ddta = $db->loadObjectList();
            }
            return $ddta;
        }
        ...
        public static function filterArray($dataArray, $keys)
        {
            $ret = array();
            if (empty($dataArray))
            {
            ...
            if (count($keys) > 1)
            {
                return self::arrayFilterFormat(self::filterArray($dataArray, array_shift($keys)), $keys[0]);
            } else
            {
                return self::arrayFilterFormat($dataArray, $keys[0]);
            }
        }

        public static function arrayFilterFormat($data, $key)
        {
            $condition = '=';
            $ret = array();
            if (empty($key->key))
            ...
            {
                case '=':
                    $ret = array_filter($data, function($d)use($key)
                    {
                        return ($d->{$key->key} == $key->value);
                    });
                    break;
                case 'in':
                    $ret = array_filter($data, function($d)use($key)
                    {
                        return ( in_array($d->{$key->key}, $key->value) );
                    });
                    break;
                default:
                    break;
        ...
        public static function checkFieldData($pid, $gid)
        {
            ...
            $key2 = new stdClass;
            $key2->key = 'access';
            $key2->value = JFactory::getUser()->getAuthorisedViewLevels();
            $key2->condition = 'in';
...
//           $fields = $db->loadObjectList();
            ...
            $fields = self::filterArray($fieldTypes, array($key1, $key2, $key3));
...
//            $fields = $db->loadObjectList();
        ...
        public static function checkTextFieldValue($pid, $fid)
        {
            ...
            } else
            {
                ...
                {
                    $db->setQuery("Select `value` from #__osrs_property_field_value where pro_id = '$propertyId' and field_id = '$fid'");
                    $value = $db->loadResult();
                    if (trim($value) == "")
                    {
                        return false;
                ...
                {
                    $db->setQuery("Select `value_integer` from #__osrs_property_field_value where pro_id = '$propertyId' and field_id = '$fid'");
                    $value = $db->loadResult();
                    if ($value > 0)
                    {
                        return true;
                ...
                {
                    $db->setQuery("Select `value_decimal` from #__osrs_property_field_value where pro_id = '$propertyId' and field_id = '$fid'");
                    $value = $db->loadResult();
                    if ($value > 0)
                    {
                        return true;
        ...
        public static function checkOptionsFieldValue($pid, $fid)
        {
            ...
            {
                $db->setQuery("Select count(id) from #__osrs_property_field_opt_value where pid = '$propertyId' and fid = '$fid'");
                $count = $db->loadResult();
                if ($count > 0)
                {
                    return true;
