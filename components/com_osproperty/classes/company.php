...
class OspropertyCompany
{
    ...
    static function display($option, $task)
    {
        ...
        switch ($task)
        {
            ...
            case "company_cancelagent":
                ...
                $mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_agent&Itemid=' . $jinput->getInt('Itemid', 0)));
                break;
        }
    }
    ...
    static function saveCompanyInfo($option)
    {
        ...
        $db = JFactory::getDbo();
        $user = JFactory::getUser();
        $country = $jinput->getString('country', HelperOspropertyCommon::getDefaultCountry());
        ...
        if (!JFolder::exists(PATH_STORE_PHOTO_COMPANY_THUMB))
            JFolder::create(PATH_STORE_PHOTO_COMPANY_THUMB);

        // remove if you want
        if (isset($post['remove_photo']))
        {
            if (is_file(PATH_STORE_PHOTO_COMPANY_FULL . DS . $post['photo']))
                unlink(PATH_STORE_PHOTO_COMPANY_FULL . DS . $post['photo']);
            if (is_file(PATH_STORE_PHOTO_COMPANY_THUMB . DS . $post['photo']))
                unlink(PATH_STORE_PHOTO_COMPANY_THUMB . DS . $post['photo']);
            $post['photo'] = '';
        }
    ...
    static function editAgent($option)
    {
        ...
        $sortby = $jinput->getString('sortby', 'a.name');
    ...
    static function companyListing($option)
    {
        ...
        $orderby = $jinput->getString('orderby', 'a.company_name');
        $ordertype = $jinput->getString('ordertype', 'asc');
        $keyword = $jinput->getString('keyword', '');
        $keyword = $db->escape($keyword);
    ...
    static function submitContact($option, $id)
    {
        ...
        if ($comment_security_code == '')
        {
            ...
            $mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_info&id=" . $id . "&Itemid=" . $jinput->getInt('Itemid', 0)), $msg);
        ...
        $mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_info&id=" . $id . "&Itemid=" . $jinput->getInt('Itemid', 0)), JText::_('OS_EMAIL_HAS_BEEN_SENT'));
    }
    ...
    static function saveAgent($option, $id, $save)
    {
        ...
        if ($id == 0)
        {
            ...
            if ($countuser > 0)
            {
                $mainframe->redirect(Jroute::_("index.php?option=com_osproperty&task=company_addagents&Itemid=" . $itemid), JText::_('OS_USER_IS_ALREADY_EXISTS'));
            }
            ...
            if ($countemail > 0)
            {
                $mainframe->redirect(Jroute::_("index.php?option=com_osproperty&task=company_addagents&Itemid=" . $itemid), JText::_('OS_EMAIL_IS_ALREADY_EXISTS'));
            }
            ...
            if (!$user->bind($data))
            {
                $mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_addagents&Itemid=' . $itemid), JText::sprintf('COM_USERS_REGISTRATION_BIND_FAILED', $user->getError()));
                return false;
            }
            // Store the data.
            if (!$user->save())
            {
                $mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_addagents&Itemid=' . $itemid), JText::sprintf('COM_USERS_REGISTRATION_SAVE_FAILED', $user->getError()));
                return false;
            }
        }

        $country = $jinput->getString('country', $configClass['show_country_id']);
        jimport('joomla.filesystem.file');
        $post = $jinput->post->getArray();

        // check folder to upload
        if (!JFolder::exists(PATH_STORE_PHOTO_AGENT_THUMB))
            JFolder::create(PATH_STORE_PHOTO_AGENT_THUMB);

        // remove file if you want
        if (isset($post['remove_photo']))
        {
            if (is_file(PATH_STORE_PHOTO_AGENT_FULL . DS . $post['photo']))
                unlink(PATH_STORE_PHOTO_AGENT_FULL . DS . $post['photo']);
            if (is_file(PATH_STORE_PHOTO_AGENT_THUMB . DS . $post['photo']))
                unlink(PATH_STORE_PHOTO_AGENT_THUMB . DS . $post['photo']);
            $post['photo'] = '';
        }
    ...
    static function manageProperties($option)
    {
        ...
        $filter_search = $jinput->getString('filter_search', '');
        $filter_search = $db->escape($filter_search);
        $sortby = $jinput->getString('sortby', 'a.id');
        $orderby = $jinput->getString('orderby', 'desc');
    ...
    static function companyRegister($option)
    {
        ...
        {
            $lists['state'] = HelperOspropertyCommon::makeStateListAddProperty('', $state, 'state', 'onchange="loadCity(this.value,\'' . $city . '\')"', JText::_('OS_SELECT_STATE'), '');
        }
        ...
        $lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option, $default_state, $city);
    ...
    static function saveNewCompany($option)
    {
        ...
        if ($configClass['show_company_captcha'] == 2)
        {
            ...
            if (!$res[0])
            {
                $mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid=' . $jinput->getInt('Itemid', 0)), JText::_('OS_SECURITY_CODE_IS_WRONG'));
            }
        }
