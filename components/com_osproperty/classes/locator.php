...
class OspropertyLocator
{
    ...
    static function locatorSearch($option)
    {
        ...
        $my_lat = $jinput->getString('my_lat', '');
        $my_long = $jinput->getString('my_long', '');
        $locator_type = $jinput->getString('locator_type', $locator_type_ids);
        ...
        $categoryArr = $jinput->get('categoryArr', array(), 'ARRAY');
        ...
        if ($property_type > 0)
        {
            $where .= " AND a.pro_type = '$property_type'";
            $no_search = false;
        }
        //}
        ...
        if ($default_search)
            $no_search = false;
        if (( ($location != '') or ( ($my_lat != "") && ($my_long != "")) ) and ( $radius_search > 0))
        {
            //echo $status;
            if ($status == "OK")
            {
        ...
        $group_by = "";
        if (($dosearch == 1) and ( ($location != '') or ( ($my_lat != "") && ($my_long != "")) ))
        {
        ...
        HTML_OspropertyLocator::locatorSearchHtml($option, $rows, $lists, $locator_type, $search_lat, $search_long);
    }
