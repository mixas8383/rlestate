...
class Plugin_googleDirections_base extends Plugin_googleMaps_base
{
    ...
    function process_params(&$row, $matches, $map_id)
    {
        ...
        if (preg_match('/dir_width=(\d+%?)/', $matches[1], $matches2))
            $this->dir_width = $matches2[1];
        if (preg_match('/mode=([^\s]+)/', $matches[1], $matches2))
            $this->mode = $matches2[1];

        if (preg_match('/from="([^"]+)"/', $this->fix_str2($matches[1]), $matches2))
            $this->from = $this->fix_str2($matches2[1]);
        if (preg_match('/to="([^"]+)"/', $this->fix_str2($matches[1]), $matches2))
            $this->to = $this->fix_str2($matches2[1]);
        #$from = str_replace('~', '@', $from);
        #$to = str_replace('~', '@', $to);
        $this->from = $this->fix_str3($this->from);
        $this->to = $this->fix_str3($this->to);

        if (preg_match('/use_own_css=(\d+)/', $matches[1], $matches2))
            $this->use_own_css = $matches2[1];
        if (preg_match('/css=([^\s]+)/', $matches[1], $matches2))
            $this->css = $matches2[1];
        if (preg_match('/dir_on_right=(\d+)/', $matches[1], $matches2))
            $this->dir_on_right = $matches2[1];
        if (preg_match('/map_on_right=(\d+)/', $matches[1], $matches2))
            $this->map_on_right = $matches2[1];
        if (preg_match('/hide_direction_text=(\d+)/', $matches[1], $matches2))
            $this->hide_direction_text = $matches2[1];
        ...
        if (preg_match('/stopover="([^"]+)"/', $this->fix_str2($matches[1]), $matches2))
            $this->stopover = $this->fix_str2($matches2[1]);
        $this->stopover = $this->fix_str3($this->stopover);

        if (preg_match('/vertical=(\d+)/', $matches[1], $matches2))
            $this->vertical = $matches2[1];
        if (preg_match('/map_full_width=(\d+)/', $matches[1], $matches2))
            $this->map_full_width = $matches2[1];
        if (preg_match('/unit=(metric|imperial)/i', $matches[1], $matches2))
            $this->unit = strtoupper($matches2[1]);

        if ($this->vertical && preg_match('/width=(\d+%)/', $matches[1], $matches2))
            $this->width = $matches2[1];
...
class Plugin_googleDirections extends Plugin_googleDirections_base
{
    ...
    function output_map(&$row, $matches, $map_id)
    {
        ...
        if ($this->width < 10 || $this->width > 4096)
            $this->width = 400;
        if ($this->dir_width < 10 || $this->dir_width > 4096)
            $this->dir_width = 275;
        ...
        if (preg_match('/%/', $this->dir_width))
            $dir_width2 = $this->dir_width;
        else
            $dir_width2 = $this->dir_width . 'px';
